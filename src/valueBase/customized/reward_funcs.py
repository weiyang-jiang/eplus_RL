...
def stpt_viol_energy_reward_part1_weiyang_simple_v1(ob_this_prcd, action_this_prcd, ob_next_prcd_raw, pcd_state_limits, e_weight, p_weight,
                                     stpt_violation_scl):
    """
    Get the reward from hvac energy and indoor air temperature setpoint violation level (the max one
    is used for the multi-zone case).
    ...
    ob_next_prcd_raw = np.array(ob_next_prcd_raw)
    min_max_limits = np.array(pcd_state_limits)
    # Do min-max normalization
    ob_next_prcd = (ob_next_prcd_raw - min_max_limits[0, 2:]) / (min_max_limits[1, 2:] -
                                                          min_max_limits[0, 2:])
    TIMESTATE_LEN = 0
    ZONE_NUM = 22
    IAT_FIRST_RAW_IDX = 4
...
reward_func_dict = {'1': err_energy_reward_iw,
                    ...
                    'part1_weiyang_simple_v1': stpt_viol_energy_reward_part1_weiyang_simple_v1,
