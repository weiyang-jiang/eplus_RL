...
class Agent_test(object):
    ...
    def complie_dqn(self):
        ...
        if self.train_dqn is not None:
            self.dqn.load_state_dict(self.train_dqn.state_dict())
        else:
    ...
    def test(self):
        ...
        self._local_logger.info(f"{self.env_name} Reward: {score} energy:{energy_total} comfort:{comfort_total}")
        self.env.close()
        if self.is_process == False:
            ...
            data_list = {"Test Reward":score, "Energy Total":energy_total, "Temperature Not Met":comfort_total}
            return data_list
