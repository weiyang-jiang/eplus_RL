...
def test():
    ...
    env_list = args.env + args.test_env
    data_list = [args.model_name, args.model_path]
    for env in env_list:
        data_list.append(str(resultparser.energy_dict_data[env]))
        data_list.append(str(resultparser.comfort_dict_data[env]))
    save_csv(data_list)

def save_csv(data_list):
    import shutil
    str_data = ",".join(data_list).replace("nan", "")
    with open(os.path.dirname(srcPath) + "/Q_learning_data/QLearnData_test.csv", "a+") as f:
        f.write("\r\n" + str_data)
    shutil.copy(os.path.dirname(srcPath) + "/Q_learning_data/QLearnData_test.csv",
                os.path.dirname(srcPath) + "/Q_learning_data/QLearnData_test_copy.csv")
