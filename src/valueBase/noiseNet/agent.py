...
from valueBase.Asyn_agent_main import AsynAgentMain, AgentMain
...
class AsynNoiseAgent(AsynAgentMain):

    def complie_agent(self):
        self.Agent_test = Noise_Agent_test
    ...
    def complie_dqn(self):
        self.dqn = Network(
            self.hist_state_dim, self.action_dim, self.hidden_size, self.noise_net_std
        ).to(self.device)
