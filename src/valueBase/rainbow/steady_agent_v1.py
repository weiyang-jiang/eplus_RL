"""
   ...
   @Date: 2022-03-31 10:17:36
"""
"""
    steady agent 是只取稳态数据放入buffer里面，只在稳态数据下进行优化和控制action
    稳态数据的定义是在一段时间内的温度波动不超过5%（可以自定义）的transition，
    如果数据不处于稳态状态下，那么就不存储当前transition，而且控制action和上一时刻相同，就是不进行控制。
    这种环境下就不需要考虑滑窗state了
    
    在测试环境下，state也进行改变，但是不考虑稳态数据直接进行控制。
    
    state数据也进行改变（只取22个zone中的最高温度，以及相对应的cooling和heating setpoint）
"""
import os
import time
...
from valueBase.rainbow.steadyAgent_v1_test import Steady_Agent_test
...
from valueBase.util.preprocessors import SteadyHistoryPreprocessor
...
class Rainbow_Agent(object):
    ...
    def __init__(
        ...
        self.histProcessor = SteadyHistoryPreprocessor(window_len, forecast_len, prcdState_dim)
        ...
        self.hist_state_dim = self.state_dim
        self.memory = memory
    ...
    def complie_agent(self):
        self.Agent_test = Steady_Agent_test
    ...
    def train(self, num_frames):
        ...
        pre_flag = True
        hist_state, state_flag = self.histProcessor. \
            process_state_for_network(state, pre=pre_flag)  # 2-D array
        ...
        for frame_idx in iter_tqdm:  # 开启训练
            ...
            next_hist_state, state_flag = self.histProcessor. \
                process_state_for_network(next_state, pre=pre_flag)  # 2-D array
            ...
            if state_flag:
                ...
                fraction = min(beta_index / num_frames, 1.0)
            ...
            if len(self.memory) >= self.history_size and state_flag == True:  # 首先初始化一段时间，然replay_buffer里面有足够的数据。
...
class SteadyAgentMainRainbow(object):
    def __init__(
            self,
            # Adam
            feature,
