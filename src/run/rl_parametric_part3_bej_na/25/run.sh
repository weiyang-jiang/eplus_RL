...
--env Part3-NA-Bej-Train-v1 `# 训练环境的名称(training environment name)` \
--device cpu `# 训练使用的设备名称(training device name)` \
--num_frames 1500000 `# 总的训练步数(total training steps)` \
--reward_func part3_v3 `# 奖励函数(reward function)` \
--metric_func part3_v1 `# 评价函数(metric function)` \
--eval_action_func cslDxActCool_1 `# 测试动作函数(eval action function)` \
--action_space part3_v1 `# 动作的大小(action space)` \
--test_env Part3-NA-Bej-Test-v1 Part3-NA-Bej-Test-v2 Part3-NA-Bej-Test-v3 Part3-NA-Bej-Test-v4 `# 测试环境名称(test environment name)` \
...
--state_dim 17 `# 状态大小(state dimension)` \
