...
--env IW-tmy3Weather-v9603 `# 训练环境的名称(training environment name)` \
--device cpu `# 训练使用的设备名称(training device name)` \
--num_frames 1500000 `# 总的训练步数(total training steps)` \
--reward_func 10 `# 奖励函数(reward function)` \
--metric_func cslDxCool_1 `# 评价函数(metric function)` \
--eval_action_func cslDxActCool_1 `# 测试动作函数(eval action function)` \
--action_space iw_af5_1 `# 动作的大小(action space)` \
--test_env IW-realWeather-v9603 `# 测试环境名称(test environment name)` \
