...
--feature rainbow_v1_statev1_35_5_1 `# 此次训练的自定义特征名称：如使用rainbow方法采用rewardv1奖励时[rainbow_v1](The custom feature for this training)` \
--method ASYNRAINBOW `# 此次训练的方法(Algorithm for this training)` \
...
--history_size 800 `# 训练开始的步数(training start step)` \
...
--num_frames 10000 `# 总的训练步数(total training steps)` \
...
--memory_size 2000 `# 缓冲器的存储大小(Replaybuffer memory size)` \
--batch_size 32 `# 训练batch的大小(batch size)` \
--target_update 200 `# 目标网络的更新间隔(update interval for target network)` \
