...
--feature Asynrainbow_v1_statev1_steady_v2 `# 此次训练的自定义特征名称：如使用rainbow方法采用rewardv1奖励时[rainbow_v1](The custom feature for this training)` \
--method STEADY `# 此次训练的方法(Algorithm for this training)` \
--process_raw_state_cmbd part1_simple_v1 `# 正则化处理state的函数(normalized state function)` \
--window_len 6 `# state的滑动窗口大小(state window length)` \
...
--reward_func part1_weiyang_simple_v1 `# 奖励函数(reward function)` \
...
--state_dim 8 `# 状态大小(state dimension)` \
