python ../../../../valueBase/test_run.py `# 训练执行文件(train main file)` \
--dir_path rl_parametric_part1_pit_light/Rainbow/weiyang_v1/Eplus-env-Asyn_Train_environment-Asynrainbow_weiyangv1_newEP_35-res1 `# ` \
--model_name rainbow_weiyangv1_35 `# 模型保存的名称 ` \
--feature Asynrainbow_weiyangv1_newEP_35 `# 此次训练的自定义特征名称：如使用rainbow方法采用rewardv1奖励时[rainbow_v1](The custom feature for this training)` \
...
--e_weight 0.5 `# 奖励函数中能耗的比重(energy weight in reward function)` \
--p_weight 0.5 `# 奖励函数中舒适度的比重(comfort weight in reward function)` \
