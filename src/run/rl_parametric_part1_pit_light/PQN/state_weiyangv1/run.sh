...
--feature PQN_weiyangv1_state_35 `# 此次训练的自定义特征名称：如使用rainbow方法采用rewardv1奖励时[rainbow_v1](The custom feature for this training)` \
--method PQN `# 此次训练的方法(Algorithm for this training)` \
--process_raw_state_cmbd part1_state_v1 `# 正则化处理state的函数(normalized state function)` \
...
--reward_func part1_weiyang_state_v1 `# 奖励函数(reward function)` \
