...
--env Model1-Cool-v2 `# 训练环境的名称(training environment name)` \
...
--action_space cslDxCool_4 `# 动作的大小(action space)` \
--test_env Model1-Test-Cool-v2 `# 测试环境名称(test environment name)` \
...
--state_dim 50 `# 状态大小(state dimension)` \
