...
--env Part4-Light-Bej-Train-Repeat-v2 `# 训练环境的名称(training environment name)` \
--device cpu `# 训练使用的设备名称(training device name)` \
--num_frames 1500000 `# 总的训练步数(total training steps)` \
--reward_func part4_heuri_v8 `# 奖励函数(reward function)` \
--metric_func part4_v2 `# 评价函数(metric function)` \
--eval_action_func cslDxActCool_1 `# 测试动作函数(eval action function)` \
--action_space part4_v3 `# 动作的大小(action space)` \
--test_env Part4-Light-Bej-Test-Repeat-v3 Part4-Light-Bej-Test-Repeat-v4 `# 测试环境名称(test environment name)` \
...
--state_dim 11 `# 状态大小(state dimension)` \
