...
--env Part4-Heavy-Pit-Train-Repeat-Cmp97-v1 `# 训练环境的名称(training environment name)` \
--device cpu `# 训练使用的设备名称(training device name)` \
--num_frames 1500000 `# 总的训练步数(total training steps)` \
--reward_func part4_heuri_cmp97 `# 奖励函数(reward function)` \
--metric_func part4_cmp97 `# 评价函数(metric function)` \
--eval_action_func cslDxActCool_1 `# 测试动作函数(eval action function)` \
--action_space iw_af4_3 `# 动作的大小(action space)` \
--test_env Part4-Heavy-Pit-Test-Repeat-Real97-v1 `# 测试环境名称(test environment name)` \
...
--state_dim 13 `# 状态大小(state dimension)` \
