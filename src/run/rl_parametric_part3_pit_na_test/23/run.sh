...
--env Part3-NA-Pit-Train-v1 `# 训练环境的名称(training environment name)` \
--device cpu `# 训练使用的设备名称(training device name)` \
--num_frames 1500000 `# 总的训练步数(total training steps)` \
--reward_func part3_v2 `# 奖励函数(reward function)` \
...
--test_env Part3-NA-Pit-Test-v1 Part3-NA-Pit-Test-v2 Part3-NA-Pit-Test-v3 Part3-NA-Pit-Test-v4 `# 测试环境名称(test environment name)` \
